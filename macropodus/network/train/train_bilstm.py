...
def train_albert(hyper_parameters=None, rate=1.0):
    ...
    if not hyper_parameters:
        ...
        'model': {'label': 575,  # 类别数
                  ...
                  'path_model_dir': os.path.join(path_model_dir, "bilstm"),
...
def train_w2v(hyper_parameters=None, rate=1.0, path_train=None, path_val=None, path_train_dir=None):
    ...
    if not hyper_parameters:
        hyper_parameters = {
            'len_max': 128,  # 句子最大长度, 固定推荐20-50, bert越长会越慢, 占用空间也会变大, 本地win10-4G设为20就好, 过大小心OOM
            'embed_size': 256,  # 768,  # 字/词向量维度, bert取768, word取300, char可以更小些
            'vocab_size': 20000,  # 这里随便填的，会根据代码里修改
            'trainable': True,  # embedding是静态的还是动态的, 即控制可不可以微调
            'level_type': 'char',  # 级别, 最小单元, 字/词, 填 'char' or 'word', 注意:word2vec模式下训练语料要首先切好
            ...
            'data': {'train_data': path_tag_people_1998_train, # path_tag_people_2014_train, # path_seg_pku_1998_train,  # path_ner_people_1998_train, # 训练数据
                     'val_data': path_tag_people_1998_train # path_tag_people_2014_valid, # path_seg_pku_1998_train  # path_ner_people_1998_valid    # 验证数据
                     },
        }
